...
func (b *Branch) Run() error {
	...
	baseLevel := logx.ResolveLevel(b.config.LogLevel, b.config.LogLevelSet, "", envx.First("LOG_LEVEL", "AYNIG_LOG_LEVEL"), config.Default().LogLevel)
	...
	resolvedLevel := logx.ResolveLevel(b.config.LogLevel, b.config.LogLevelSet, trailerLevel, envx.First("LOG_LEVEL", "AYNIG_LOG_LEVEL"), config.Default().LogLevel)
