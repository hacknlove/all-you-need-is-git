...
```bash
aynig set-state --state <state> [options]
```
...
- The push remote is resolved from `--remote` first, then from the `dwp-source` trailer on `HEAD`.
...
```bash
aynig set-state --state review --subject "review: ready for feedback"
```
...
```bash
aynig set-state --state failed --prompt "Tests failed in CI; investigating flaky snapshot output."
```
...
```bash
aynig set-state --state failed --prompt-file ./failure.txt
```
...
```bash
aynig set-state --state review --remote origin
```
...
```bash
aynig set-state --state review --keep-trailers
```

### `--trailer <key:value>`

Adds an extra trailer to the commit. Repeat the flag to add more than one.
`dwp-state` cannot be set with `--trailer`; it is managed by `--state`.

Example:

```bash
aynig set-state --state triage --trailer "dwp-issue: 42"
```

Repeated example:

```bash
aynig set-state --state review --trailer "dwp-issue: 42" --trailer "dwp-note: waiting on design sign-off"
```
...
```bash
aynig set-state --state review --subject "review: ready for feedback" --prompt "Core flow is implemented and ready for another pass."
```
...
```bash
git status --short | aynig set-state --state triage --prompt-stdin
```
